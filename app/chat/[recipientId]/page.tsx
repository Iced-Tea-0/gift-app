...
export default function ChatPage({ params }: { params: { recipientId: string } }) {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      type: 'ai',
      text: 'Hi! Tell me more about this person so I can suggest the perfect gift.',
    },
  ]);
  ...
  return (
    <main className="min-h-screen overflow-hidden flex flex-col relative">
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        ...
        <div className="absolute bottom-1/4 left-1/4 text-6xl opacity-10 animate-float" style={{ animationDelay: '2s' }}>✧</div>
      </div>
      ...
      <div className="fixed top-0 left-0 right-0 z-30 flex items-center gap-4 px-8 py-6 border-b border-pink-200/40 bg-white/50 backdrop-blur-md">
        ...
        <h1 className="text-2xl font-bold text-gray-900">Planning for You 🎀</h1>
      </div>
      ...
      <div className="relative z-20 border-t border-pink-200/50 glass px-8 py-6">
        <div className="flex gap-3 max-w-4xl mx-auto">
          <input
            ...
            onKeyPress={(e) => e.key === 'Enter' && !e.nativeEvent.isComposing && handleSendMessage()}
      ...
      {savedSuccess && (
        <div className="fixed bottom-8 left-1/2 -translate-x-1/2 glass-card border-2 border-pink-300 text-gray-900 px-6 py-3 rounded-full z-50 shadow-xl font-bold animate-in slide-in-from-bottom-4">
          Perfect! Gift saved. 🎁
        </div>
      )}
    </main>
